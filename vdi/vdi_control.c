...
void vdi_v_opnwk(Vwk * vwk)
{
	...
	if(INTIN[0]==5)
	{
		if(INTOUT[45]&SAGA_ENABLE)
		{ int type;
			type=INTOUT[45]&0xF;
			if(type!=0x8 && type!=0x9 && type!=0xA) /* only ST 1,2,4 plane */
			{
				CONTRL[6] = 0;  /* Resolution not supported */
				return ;
			}
		}
	}
     ...
     * mode + 2 in INTIN[0]. Modecodes are passed through INTOUT[45].
     */

    Setscreen(-1L, -1L, INTIN[0] - 2, INTOUT[45]);
#endif

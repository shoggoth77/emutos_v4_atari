...
CPP = $(CC) -E
CPUFLAGS = -m68000
MULTILIBFLAGS = $(CPUFLAGS) -mshort -fno-builtin-strcpy
INC = -Iinclude
#OTHERFLAGS = -fomit-frame-pointer -fcommon  pour GCC<4
...
bios_src +=  memory.S processor.S vectors.S aciavecs.S bios.c xbios.c acsi.c \
             ...
             amiga.c amiga2.S spi_vamp.c spi_vamp_asm.S vproxy.c \
...
help:
	...
	@echo "v4sa-prg emutos.prg, EmuTOS ROM for Amiga Vampire V4 Standalone"
...
.PHONY: amiga amiga256 amiga512
NODEP += amiga amiga256 amiga512

amiga: amiga256
amiga256: UNIQUE = $(COUNTRY)
amiga256: ROM_AMIGA_SIZE = 256
amiga256: amigarom
amiga512: ROM_AMIGA_SIZE = 512
amiga512: amigarom
amigarom: OPTFLAGS = $(SMALL_OPTFLAGS)
amigarom: override DEF += -DTARGET_AMIGA_ROM_$(ROM_AMIGA_SIZE) $(AMIGA_DEFS)
amigarom:
	@echo "# Building Amiga EmuTOS into $(ROM_AMIGA)"
	$(MAKE) CPUFLAGS='$(CPUFLAGS)' DEF='$(DEF)' OPTFLAGS='$(OPTFLAGS)' UNIQUE=$(UNIQUE) ROM_AMIGA=$(ROM_AMIGA) ROM_AMIGA_SIZE=$(ROM_AMIGA_SIZE) $(ROM_AMIGA)
	@MEMBOT=$(call SHELL_SYMADDR,__end_os_stram,emutos.map);\
	echo "# RAM used: $$(($$MEMBOT)) bytes ($$(($$MEMBOT - $(MEMBOT_TOS206))) bytes more than TOS 2.06)"
	@printf "$(LOCALCONFINFO)"

$(ROM_AMIGA): emutos.img mkrom
	./mkrom amiga $(ROM_AMIGA_SIZE)k $< $(ROM_AMIGA)
...
amigavampire: amiga256

# Special Amiga ROM optimized for Vampire V4 Standalone
#  - 496 megs of fastram located at 0x01000000.
#  - Low 8 megs reserved for floppy emulation hw and flicker fixers
#  - Top 8 megs reserved for an 8 meg VDI screen (enough for 1920x1080x4).
V4_DEF = -DNO_AMIGA_VBI=1 -DSTATIC_ALT_RAM_ADDRESS=0x01800000 -DSTATIC_ALT_RAM_SIZE=480UL*1024*1024
#V4_DEF = -DNO_AMIGA_VBI=0 -DSTATIC_ALT_RAM_ADDRESS=0x01800000 -DSTATIC_ALT_RAM_SIZE=480UL*1024*1024
...
.PHONY: v4sa-prg
NODEP += v4sa-prg
v4sa-prg: CPUFLAGS = $(VAMPIRE_CPUFLAGS)
v4sa-prg: OPTFLAGS = $(SMALL_OPTFLAGS)
v4sa-prg: override DEF += -DMACHINE_AMIGA $(VAMPIRE_COMMON_DEF) $(V4_DEF) $(AMIGA_DEFS)
v4sa-prg:
	@echo "# Building V4SA EmuTOS $(EMUTOS_PRG)"
	$(MAKE) CPUFLAGS='$(CPUFLAGS)' DEF='$(DEF)' OPTFLAGS='$(OPTFLAGS)' UNIQUE=$(UNIQUE) prg
	@MEMBOT=$(call SHELL_SYMADDR,__end_os_stram,emutos.map);\
	echo "# RAM used: $$(($$MEMBOT)) bytes ($$(($$MEMBOT - $(MEMBOT_TOS206))) bytes more than TOS 2.06)"
	@printf "$(LOCALCONFINFO)"

...
#else
#if CONF_WITH_APOLLO_68080
    cookie_mch = MCH_VAMPIRE;/*   MCH_ARANYM; */
#else
...
#if CONF_WITH_APOLLO_68080
#if CONF_WITH_YM2149
	cookie_snd = 0x06 | SND_PSG; /* Minimal sound + xbios sound + PSG*/
#else
    cookie_snd = 0x06; /* Minimal sound + xbios sound*/
#endif
#else
    cookie_snd = 0;
...
void fill_cookie_jar_nvram(void) /* do again tests and fix cookie now nvram can be read (system without nvram as file on disk (Amiga)) */
{
    ...
    cookie_fix(COOKIE_IDT, cookie_idt);

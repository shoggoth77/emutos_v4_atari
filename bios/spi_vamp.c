...
/* Note: This code is kept as an alternative implementation, which
can be useful when debugging the SD/SPI driver later on */

#if 0 /* CONF_WITH_VAMPIRE_SPI */
...
void spi_recv_block(UBYTE *buf,UWORD len)
{
    if (buf)
    {
        while(len--)
...
void spi_send_block(UBYTE *buf,UWORD len)
{
    if(buf)
    {
        while(len--)
        {
            SAGA_SDCARD_DATA = *buf++;
            FORCE_READ(SAGA_SDCARD_DATA);
        }
    }
    else

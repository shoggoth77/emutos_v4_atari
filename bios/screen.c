...
#if CONF_WITH_APOLLO_68080
    screen_start=find_fastvideoram();
    if(!screen_start)
    {
...
void setscreen(UBYTE *logLoc, const UBYTE *physLoc, WORD rez, WORD videlmode)
{
    ...
    if ((logLoc!=(UBYTE *)-1L) && (logLoc!=(UBYTE *)NULL)) {
        v_bas_ad = logLoc;
        KDEBUG(("v_bas_ad = %p\n", v_bas_ad));
    }
    if ((physLoc!=(UBYTE *)-1L) && (physLoc!=(UBYTE *)NULL)) {
        setphys(physLoc);
    }

    if((logLoc==(UBYTE *)NULL) && (physLoc==(UBYTE *)NULL))
    {
...
{
    LONG a;
#if CONF_WITH_ATARI_VIDEO || NO_AMIGA_VBI
    ...
    /* We replicate ditto behavior for the Amiga, which does however
     * have slightly different interrupt priorities */
    WORD old_sr = set_sr(0x2200);       /* allow VBL interrupt */
#endif

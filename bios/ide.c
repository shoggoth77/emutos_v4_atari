...
#if CONF_WITH_APOLLO_68080
    ifinfo[0].base_address = ide_interface;
    ifinfo[0].twisted_cable = FALSE;
# if NUM_IDE_INTERFACES > 1
    ifinfo[1].base_address = ide_interface2;
    ifinfo[1].twisted_cable = FALSE;
# endif
#else
...
static void ide_get_data_32(volatile struct IDE *interface,UBYTE *buffer,ULONG bufferlen,int need_byteswap)
{
    ...
    if (need_byteswap) {
        ...
        while (p < end) {  /* SECTOR_SIZE = 512 bit always multiple of it */
            *p++ = *pdatareg;
            *p++ = *pdatareg;
            *p++ = *pdatareg;
            *p++ = *pdatareg;

...
void biosmain(void)
{
...
#ifdef MACHINE_AMIGA /* need because nvram can be access only after filesystem init so need update language information */
#if CONF_MULTILANG  
    fill_cookie_jar_nvram();
    KDEBUG(("font_init()\n"));
    font_init();        /* initialize font ring (requires cookie_akp) */
#if CONF_WITH_NLS
    KDEBUG(("nls_init()\n"));
    ...
    if((bootflags&(BOOTFLAG_SKIP_HDD_BOOT|BOOTFLAG_SKIP_AUTO_ACC))==0) v4_better_init();
#endif 
#endif  

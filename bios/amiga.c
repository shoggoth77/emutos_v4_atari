...
# error This fork requires CONF_WITH_VECTOR_PROXY
#endif

#if !CONF_WITH_MFP_EMULATION
//# error This fork requires CONF_WITH_MFP_EMULATION
MFP mfp_emu;
#endif
...
#define MWHEEL  *(volatile UWORD*)0xdff212
...
#define DMACON2   0xDFF296    /* (W) Control AUD DMA  (Bit0 to Bit11) AUD4..15 SAGA only */
#define AUD0LC  *(volatile ULONG*)0xdff0a0
#define AUD0LEN *(volatile UWORD*)0xdff0a4
#define AUD0PER *(volatile UWORD*)0xdff0a6
#define AUD0VOL *(volatile UWORD*)0xdff0a8
#define AUD0VOLFS *(volatile UWORD*)0xdff0ac /* vampire only volume Far Side ???*/
#define AUD0LENH *(volatile UWORD*)0xdff0ae  /* vampire only Amiga length only on 16 bits */
#define AUD1LC  *(volatile ULONG*)0xdff0b0
#define AUD1LEN *(volatile UWORD*)0xdff0b4
#define AUD1PER *(volatile UWORD*)0xdff0b6
#define AUD1VOL *(volatile UWORD*)0xdff0b8
#define AUD1VOLFS *(volatile UWORD*)0xdff0bc /* vampire only volume Far Side ???*/
#define AUD1LENH *(volatile UWORD*)0xdff0be  /* vampire only Amiga length only on 16 bits */
#define AUD2LC  *(volatile ULONG*)0xdff0c0
#define AUD2LEN *(volatile UWORD*)0xdff0c4
#define AUD2PER *(volatile UWORD*)0xdff0c6
#define AUD2VOL *(volatile UWORD*)0xdff0c8
#define AUD2VOLFS *(volatile UWORD*)0xdff0cc /* vampire only volume Far Side ???*/
#define AUD2LENH *(volatile UWORD*)0xdff0ce  /* vampire only Amiga length only on 16 bits */
#define AUD3LC  *(volatile ULONG*)0xdff0d0
#define AUD3LEN *(volatile UWORD*)0xdff0d4
#define AUD3PER *(volatile UWORD*)0xdff0d6
#define AUD3VOL *(volatile UWORD*)0xdff0d8
#define AUD3VOLFS *(volatile UWORD*)0xdff0dc /* vampire only volume Far Side ???*/
#define AUD3LENH *(volatile UWORD*)0xdff0de  /* vampire only Amiga length only on 16 bits */

#define AUD0LSAGA  *(volatile ULONG*)0xdff400   /* adresse Ã©chantillon */
#define AUD0LENSAGA  *(volatile ULONG*)0xdff404  /* longueur Ã©chantillon */
#define AUD0VOLSAGA  *(volatile UWORD*)0xdff408 /* 0x80 max, 0 min */
/* AUD0CTRLSAGA 
	bit 0 : 0 = 8 bits, 1 = 16 bits
	bit 1 : 0 = loop 1 = stop at end of sample  
	bit 2 : 0 = sample on 2 separate buffer 1 = interleaved data buffer */
#define AUD0CTRLSAGA  *(volatile UWORD*)0xdff40a
#define AUD0PERSAGA  *(volatile UWORD*)0xdff40c  /* pÃ©riode valeur mini 65 qui donne 56khz */

#define AUD1LSAGA  *(volatile ULONG*)0xdff410   /* adresse Ã©chantillon */
#define AUD1LENSAGA  *(volatile ULONG*)0xdff414  /* longueur Ã©chantillon */
#define AUD1VOLSAGA  *(volatile UWORD*)0xdff418 /* 0x80 max, 0 min */
/* AUD1CTRLSAGA 
	bit 0 : 0 = 8 bits, 1 = 16 bits
	bit 1 : 0 = loop 1 = stop at end of sample  
	bit 2 : 0 = sample on 2 separate buffer 1 = interleaved data buffer */
#define AUD1CTRLSAGA  *(volatile UWORD*)0xdff41a
#define AUD1PERSAGA  *(volatile UWORD*)0xdff41c  /* pÃ©riode valeur mini 65 qui donne 56khz */
...
ULONG oldjoy0dat;
ULONG oldjoy1dat;
void (*mouse_report_mv)(void);
void (*mouse_report_kd_left)(void);
void (*mouse_report_kd_right)(void);
void (*mouse_report_ku_left)(void);
...
extern void mouse_report_0x75(void);
extern void mouse_report_0xf4(void);
extern void mouse_report_0xf5(void);
extern void mouse_report_none(void);
extern void joy0_report_event(void);
...
#if CONF_WITH_APOLLO_68080
void v4_better_init(void)
{   int err;
    UBYTE buf[1];
    err = nvmaccess(0, 51, 1, buf);
    if (err == 0)
    {
        if(buf[0]&0x1) /* Fast IDE to set */
        {
...
void amiga_machine_init(void)
{
    vproxy_init();
    
    VPROXY_VECTORS[0x64>>2] = amiga_int_1;
#if NO_AMIGA_VBI
    // don't set vectors, let code flow directly through Atari handlers
    // vector 4 not set because it goes straight to Atari VBL
#else
    VPROXY_VECTORS[0x68>>2] = amiga_int_2;
    VPROXY_VECTORS[0x6C>>2] = amiga_int_3;
    VPROXY_VECTORS[0x70>>2] = amiga_int_4;
#endif
    VPROXY_VECTORS[0x74>>2] = amiga_int_5;
    VPROXY_VECTORS[0x78>>2] = amiga_int_6;
}
...
static UBYTE *start_motherboard_fastram, *end_motherboard_fastram,*start_slot_fastram, *end_slot_fastram;

#if !CONF_VRAM_ADDRESS
UBYTE *find_fastvideoram(void);
UBYTE *find_fastvideoram(void)
{   UBYTE *start = (UBYTE *)0x01800000;
    UBYTE *end = (UBYTE *)0x08000000; /* Maximum positive address? */
    ULONG size;
    UBYTE *fastvideoram=NULL;

    start_motherboard_fastram = (UBYTE *)0x01800000;
    end_motherboard_fastram = (UBYTE *)0x08000000;
    start_slot_fastram = (UBYTE *)0x08000000;
    end_slot_fastram = (UBYTE *)0x7f000000;
    ...
    if(size>initial_vram_size()) {
    	fastvideoram = end - initial_vram_size();
    	end_motherboard_fastram -= initial_vram_size();
    }
    else
    {	start = (UBYTE *)0x08000000;
    	end = (UBYTE *)0x7f000000;
    	size = amiga_detect_ram(start, end, 1*1024*1024UL);
    	if(size>initial_vram_size()) 
    	{
    		fastvideoram = start;
    		start_slot_fastram +=initial_vram_size() ;
    	}
    }
    return fastvideoram;
}
...
{
    UBYTE *start = start_slot_fastram;
    UBYTE *end = end_slot_fastram; /* Maximum positive address? */
    ULONG size;
...
{
    UBYTE *start = (UBYTE *)start_motherboard_fastram; /* Start of 32-bit space */
    UBYTE *end = (UBYTE *)end_motherboard_fastram;
    ULONG size;
...
#define MODECODE_TTHIGH (VIDEL_VGA | VIDEL_COMPAT | VIDEL_80COL | VIDEL_1BPP)
...
/*#define MODECODE_BOOT   (MODECODE_TTMED)*/
#define MODECODE_BOOT   (0x0A00 | SAGA_VIDEO_FORMAT_STLOW | SAGA_ENABLE)

#define SRGB_BLACK     0x00000000
#define SRGB_BLUE      0x000000ff
#define SRGB_GREEN     0x0000ff00
#define SRGB_CYAN      0x0000ffff
#define SRGB_RED       0x00ff0000
#define SRGB_MAGENTA   0x00ff00ff
#define SRGB_LTGRAY    0x00bbbbbb
#define SRGB_GRAY      0x00888888
#define SRGB_LTBLUE    0x000000aa
#define SRGB_LTGREEN   0x0000aa00
#define SRGB_LTCYAN    0x0000aaaa
#define SRGB_LTRED     0x00aa0000
#define SRGB_LTMAGENTA 0x00aa00aa
#define SRGB_YELLOW    0x00ffff00
#define SRGB_LTYELLOW  0x00aaaa00
#define SRGB_WHITE     0x00ffffff
...
static const ULONG saga_dflt_palette[] = {
    SRGB_WHITE, SRGB_RED, SRGB_GREEN, SRGB_YELLOW,
    SRGB_BLUE, SRGB_MAGENTA, SRGB_CYAN, SRGB_LTGRAY,
    SRGB_GRAY, SRGB_LTRED, SRGB_LTGREEN, SRGB_LTYELLOW,
    SRGB_LTBLUE, SRGB_LTMAGENTA, SRGB_LTCYAN, SRGB_BLACK,
    ...
    0x00ff0000, 0x00ff0011, 0x00ff0021, 0x00ff0033,
    0x00ff0044, 0x00ff0054, 0x00ff0066, 0x00ff0077,
    0x00ff0087, 0x00ff0099, 0x00ff00aa, 0x00ff00ba,
    0x00ff00cc, 0x00ff00dd, 0x00ff00ed, 0x00ff00ff,
    0x00ed00ff, 0x00dd00ff, 0x00cc00ff, 0x00ba00ff,
    0x00aa00ff, 0x009900ff, 0x008700ff, 0x007700ff,
    0x006600ff, 0x005400ff, 0x004400ff, 0x003300ff,
    0x002100ff, 0x001100ff, 0x000000ff, 0x000011ff,
    0x000021ff, 0x000033ff, 0x000044ff, 0x000054ff,
    0x000066ff, 0x000077ff, 0x000087ff, 0x000099ff,
    0x0000aaff, 0x0000baff, 0x0000ccff, 0x0000ddff,
    0x0000edff, 0x0000ffff, 0x0000ffed, 0x0000ffdd,
    0x0000ffcc, 0x0000ffba, 0x0000ffaa, 0x0000ff99,
    0x0000ff87, 0x0000ff77, 0x0000ff66, 0x0000ff54,
    0x0000ff44, 0x0000ff33, 0x0000ff21, 0x0000ff11,
    0x0000ff00, 0x0011ff00, 0x0021ff00, 0x0033ff00,
    0x0044ff00, 0x0054ff00, 0x0066ff00, 0x0077ff00,
    0x0087ff00, 0x0099ff00, 0x00aaff00, 0x00baff00,
    0x00ccff00, 0x00ddff00, 0x00edff00, 0x00ffff00,
    0x00ffed00, 0x00ffdd00, 0x00ffcc00, 0x00ffba00,
    0x00ffaa00, 0x00ff9900, 0x00ff8700, 0x00ff7700,
    0x00ff6600, 0x00ff5400, 0x00ff4400, 0x00ff3300,
    0x00ff2100, 0x00ff1100, 0x00ba0000, 0x00ba0011,
    0x00ba0021, 0x00ba0033, 0x00ba0044, 0x00ba0054,
    0x00ba0066, 0x00ba0077, 0x00ba0087, 0x00ba0099,
    0x00ba00aa, 0x00ba00ba, 0x00aa00ba, 0x009900ba,
    0x008700ba, 0x007700ba, 0x006600ba, 0x005400ba,
    0x004400ba, 0x003300ba, 0x002100ba, 0x001100ba,
    0x000000ba, 0x000011ba, 0x000021ba, 0x000033ba,
    0x000044ba, 0x000054ba, 0x000066ba, 0x000077ba,
    0x000087ba, 0x000099ba, 0x0000aaba, 0x0000baba,
    0x0000baaa, 0x0000ba99, 0x0000ba87, 0x0000ba77,
    0x0000ba66, 0x0000ba54, 0x0000ba44, 0x0000ba33,
    0x0000ba21, 0x0000ba11, 0x0000ba00, 0x0011ba00,
    0x0021ba00, 0x0033ba00, 0x0044ba00, 0x0054ba00,
    0x0066ba00, 0x0077ba00, 0x0087ba00, 0x0099ba00,
    0x00aaba00, 0x00baba00, 0x00baaa00, 0x00ba9900,
    0x00ba8700, 0x00ba7700, 0x00ba6600, 0x00ba5400,
    0x00ba4400, 0x00ba3300, 0x00ba2100, 0x00ba1100,
    0x00770000, 0x00770011, 0x00770021, 0x00770033,
    0x00770044, 0x00770054, 0x00770066, 0x00770077,
    0x00660077, 0x00540077, 0x00440077, 0x00330077,
    0x00210077, 0x00110077, 0x00000077, 0x00001177,
    0x00002177, 0x00003377, 0x00004477, 0x00005477,
    0x00006677, 0x00007777, 0x00007766, 0x00007754,
    0x00007744, 0x00007733, 0x00007721, 0x00007711,
    0x00007700, 0x00117700, 0x00217700, 0x00337700,
    0x00447700, 0x00547700, 0x00667700, 0x00777700,
    0x00776600, 0x00775400, 0x00774400, 0x00773300,
    0x00772100, 0x00771100, 0x00440000, 0x00440011,
    0x00440021, 0x00440033, 0x00440044, 0x00330044,
    0x00210044, 0x00110044, 0x00000044, 0x00001144,
    0x00002144, 0x00003344, 0x00004444, 0x00004433,
    0x00004421, 0x00004411, 0x00004400, 0x00114400,
    0x00214400, 0x00334400, 0x00444400, 0x00443300,
    0x00442100, 0x00441100, SRGB_WHITE, SRGB_BLACK
};
...
GLOBAL ULONG saga_shadow_palette[256];   /* real Falcon does this, used by vectors.S */
...
ULONG amiga_initial_vram_size(void)
{
	...
	for(i = 0; i < ARRAYSIZE(saga_fmt_info); i++)
	{
		ULONG tmp = saga_fmt_info[i].bpp;

		if(tmp > bpp)
			bpp = tmp;
	}

	for(i = 0; i < ARRAYSIZE(saga_res); i++)
	{
		ULONG tmp = (long)saga_res[i].width * (long)saga_res[i].height;
...
{
	calc_modecode_info(vsetmode(-1), planes, hz_rez, vt_rez);
}

void amiga_screen_init(void)
{
    amiga_setrez(FALCON_REZ, MODECODE_BOOT);

    /* Enable VBL interrupt */
    VEC_VBL = int_vbl;

#if NO_AMIGA_VBI
    INTENA = CLRBITS | INTEN | VERTB ;
#else
     ...
     *(volatile UWORD *)SAGA_VIDEO_SPRITEX = SAGA_VIDEO_MAXHV - 1;
     *(volatile UWORD *)SAGA_VIDEO_SPRITEY = SAGA_VIDEO_MAXVV - 1;
}
...
WORD amiga_setcolor(WORD index, WORD value)
{
	union irgb tmp;
	...
	retval = ste_shadow_palette[index];
	ste_shadow_palette[index] = value;

	UWORD col = ((value & 0x0777) << 1) | ((value & 0x0888) >> 3);

	tmp.comp.r = (col & 0x0F00) >> 4;
	tmp.comp.g = (col & 0x00F0) >> 0;
	tmp.comp.b = (col & 0x000F) << 4;

	tmp.comp.r |= tmp.comp.r >> 4;
	tmp.comp.g |= tmp.comp.g >> 4;
	tmp.comp.b |= tmp.comp.b >> 4;

	tmp.comp.index = index;

	SAGA_VIDEO_PALV4SA = tmp.full; 
...
WORD vsetrgb(WORD index,WORD count,const ULONG *rgb)
{
    ...
    amiga_get_current_mode_info(&bpp, NULL, NULL);
    limit = (bpp <= 8) ? (1 << bpp) : 256;

    if ((index + count) > limit)
        return -1; /* Generic error */

    shadow = saga_shadow_palette + index;
    while(count--) {
        u.l = *rgb++;
    ...
    colorptr = (limit==256) ? (UWORD *)0x01L : (UWORD *)((LONG)saga_shadow_palette|0x01L);
...
{
    static WORD cur_modecode;
    WORD prev_modecode;

    if (mode == -1) 
        return cur_modecode;

    prev_modecode = cur_modecode;
    cur_modecode  = mode;
	...
	if(mode & SAGA_ENABLE)
	{
		SAGA_SET_VIDEO_MODE = mode & ~SAGA_ENABLE;
    ...
    SAGA_SET_CONTROL_REG = 0x0001; /* disable scanline effect */
...
void amiga_setrez(WORD rez, WORD videlmode)
{
	...
	{
		default:
            rez = ST_LOW; /* Fallthrough */

		case ST_LOW:    videlmode = MODECODE_STLOW;  break;
		case ST_MEDIUM:	videlmode = MODECODE_STMED;  break;
        case ST_HIGH:   videlmode = MODECODE_STHIGH; break;
        case TT_LOW:    videlmode = MODECODE_TTLOW;  break;
		case TT_MEDIUM: videlmode = MODECODE_TTMED;  break;
        case TT_HIGH:   videlmode = MODECODE_TTHIGH; break;
		...
		case FALCON_REZ:
        {
            if(videlmode & VIDEL_COMPAT)
            {
                ...
                {
                    default:
                    case VIDEL_1BPP: rez = ST_HIGH;   break;
                    case VIDEL_2BPP: rez = ST_MEDIUM; break;
                    case VIDEL_4BPP: rez = (videlmode & VIDEL_80COL) ? TT_MEDIUM : ST_LOW; break;
                    case VIDEL_8BPP: rez = TT_LOW;    break;
                }
            }
        }
            break;
	...
	calc_modecode_info(videlmode, &bpp, NULL, NULL);

	if(bpp <= 8)
        vsetrgb(0, 1 << bpp , (bpp == 1) ? &saga_dflt_palette[254] : &saga_dflt_palette[0]);
...
typedef struct
{
	ULONG sound_startaddr;
	ULONG sound_endaddr;
	ULONG last_sound_addr;
	ULONG current_sound_addr;
	UWORD sound_mode;
	UWORD sound_irqmode;
/*	UWORD sound_irqcause;*/
	UWORD gpio_data;
	UWORD sound_rate;
/*	WORD sound_dest;*/
/*	WORD sound_source;*/
...
/*	WORD sound_protocol;*/
	WORD sound_vol;
	WORD sound_vol_atari;
	WORD sound_shift;
	WORD sound_state;
	WORD soundcmd_prescale;
	WORD start_song;
	volatile WORD put_sound;
	WORD oneshot;
	
	ULONG bufferaddi;  /* need to be sure buffer address is align depending of the mode */
	ULONG buffermask;

} XBIOS_SOUND_CONF;

static XBIOS_SOUND_CONF sound_conf;
unsigned short play_int_ch0;
#ifdef WITH_END_BUFFER
long endbuffer[16];
#endif
/******************************************************************************/
/* Extra Amiga sound IRQ                                                      */
/******************************************************************************/
/*static void __attribute__((interrupt)) dummy_int( void )
{
}*/
void (*sound_int_call)(void);
void (*rec_int_call)(void);
void amiga_extra_sound(void)
{ 
  INTREQ= CLRBITS | AUD0;

  sound_conf.current_sound_addr = sound_conf.last_sound_addr;
  if(sound_conf.start_song==1 && play_int_ch0==1) 
  {
  	play_int_ch0=0;
  	sound_conf.start_song=2;
  }
  else if(sound_conf.start_song==2) 
  {
  	play_int_ch0=1;
  	sound_conf.start_song=0;
  }
#ifdef WITH_END_BUFFER
  if(sound_conf.current_sound_addr==((ULONG)endbuffer+7L)&0xFFFFFFF8L)
  {
      	    INTENA = CLRBITS | AUD0; /* stop interrupt */
    	    DMACON = CLRBITS | AUD0EN | AUD1EN; /*0x0001 Arret canal 0 */
  }
#endif

  if(sound_conf.sound_startaddr!=sound_conf.last_sound_addr)
  {
    	AUD0LSAGA = sound_conf.sound_startaddr;
    	AUD0LENSAGA = (sound_conf.sound_endaddr-sound_conf.sound_startaddr)>>(sound_conf.sound_shift); /* nombre de mots */
    	sound_conf.last_sound_addr = sound_conf.sound_startaddr;
    	sound_conf.put_sound=0;
   }
   else 
   {
   	sound_conf.put_sound=1;
   }

}


void amiga_soundinit(void)
{   
    DMACON = CLRBITS | AUD0EN | AUD1EN; /*0x0001 Arret canal 0 et 1 */
    INTENA = CLRBITS | AUD0 | AUD1;
    mfp_emu.tacr &= ~0x08;
    sound_int_call = NULL;
    play_int_ch0=0;
    rec_int_call = NULL;
    sound_conf.sound_startaddr = 0L;
    sound_conf.current_sound_addr = 0L;
    sound_conf.sound_endaddr = 0L;
    sound_conf.last_sound_addr = 0L;
    
    sound_conf.sound_irqmode = 0;
 /*   sound_conf.sound_irqcause = 0;*/
    
    sound_conf.sound_clk = 0;
    sound_conf.sound_prescale = 3;
    sound_conf.soundcmd_prescale = 1;
/*    sound_conf.sound_protocol = 1; */
    sound_conf.sound_rate = 0;
    sound_conf.sound_state = 0;
    sound_conf.gpio_data = 3;
    sound_conf.put_sound=1;
    sound_conf.oneshot=0;
    sound_conf.sound_vol=0x8080;
    sound_conf.sound_vol_atari=0x0FF0;
    sound_conf.sound_mode = 0;
    sound_conf.sound_shift = 1;
    sound_conf.start_song=0;
    sound_conf.bufferaddi=0;
    sound_conf.buffermask=0xFFFFFFFFL;
#ifdef WITH_END_BUFFER
    while(i<16) endbuffer[i++]=0L;
#endif
}

static ULONG align_buffer_add(ULONG address)
{
	return (address+sound_conf.bufferaddi)&sound_conf.buffermask;
}

LONG amiga_setbuffer(UWORD mode, ULONG startaddr, ULONG endaddr)
{   
    if(mode!=0) return EBADRQ;
    

    if(sound_conf.sound_mode==0)
    { unsigned short *sound=(unsigned short *)startaddr;
    	while ((unsigned long)sound < endaddr) /* stereo is not support in 8 bits, clean sound for mono only */
    	{
    		*sound++&=0xFF00;
    	}
    }

    if(sound_conf.put_sound)
    {
    	sound_conf.put_sound=0;
    	sound_conf.last_sound_addr = sound_conf.sound_startaddr = align_buffer_add(startaddr);
    	sound_conf.sound_endaddr = endaddr;
    ...
    {
    	sound_conf.sound_startaddr = align_buffer_add(startaddr);
    	sound_conf.sound_endaddr = endaddr;
    }
...
LONG amiga_setinterrupt(UWORD mode, WORD cause, void *intr_addr)
{
	if(mode < 2)
	{
	  sound_conf.sound_irqmode = mode;
	/*  sound_conf.sound_irqcause = cause;*/
	  if(mode==0 && cause==0) 
	  {	
	  	/*	sound_conf.sound_state = 0;*/
	  	jdisint(13);
	  ...
	  {
	  	mfp_emu.tacr |= 0x08;
	  	jenabint(13); /* start interrupt sound chanel 0 */
	  }
	}
	if(mode==2) /* set interrupt GSXB extension */
	{
		
		
		if (cause==0) 
		{
			if(sound_conf.sound_irqmode==0) 
			{
				if(mfp_emu.tacr &0x08) jdisint(13);  /* stop interrupt sound chanel 0 */
				else jdisint(15);			
			}
		...
		{	
			if (intr_addr !=NULL) 
			{
				if(cause&1)
				{
					if(sound_conf.sound_irqmode==0) 
					{
						sound_int_call = (LONG) intr_addr;
						
						if(mfp_emu.tacr &0x08) 
						{
							*(LONG *)((0x40L + 13)*4) = (LONG) intr_addr;
							jenabint(13);
						}
						else 
						{
							*(LONG *)((0x40L + 15)*4) = (LONG) intr_addr;
							jenabint(15); /* start interrupt sound chanel 0 */
						}
					}
...
LONG amiga_setmode(UWORD mode)
{	LONG rep=0;
    sound_conf.sound_mode = mode&0x0F; /* 0: 8 bits stereo 1: 16 bits stereo 2: 8 bits mono 3: 16 bits mono*/
    switch (mode&0x0F)  /*mode is selected in the low byte of Mode, and the desired capture mode is selected in the high byte*/
    {
    	case 0:
    	 sound_conf.sound_shift = 2;
    	 sound_conf.bufferaddi=3;
    	 sound_conf.buffermask=0xFFFFFFFCL;
    	break;
    	case 1:
    	 sound_conf.sound_shift = 3;
    	 sound_conf.bufferaddi=7;
    	 sound_conf.buffermask=0xFFFFFFF8L;
    	break;
    	case 2:
         sound_conf.sound_shift = 1;
         sound_conf.bufferaddi=1;
         sound_conf.buffermask=0xFFFFFFFEL;
    	break;
    	case 3:
    	 sound_conf.sound_shift = 2;
    	 sound_conf.bufferaddi=3;
...
LONG amiga_gpio(UWORD mode, UWORD data)
{	static UWORD olddata=0;
	switch(mode)
	{
		case 0: return olddata;
		break;
		case 1: return sound_conf.gpio_data;
		break;
		case 2:
			olddata = sound_conf.gpio_data;
			sound_conf.gpio_data = data&7;
		break;
		default:
		break;
...
LONG amiga_devconnect(WORD source, WORD dest, WORD clk, WORD prescale, WORD protocol)
{
    if(source!=0) return 1; /* DMA only */
  /*  if(!(dest & 0x01)) return 1 ;*/
    if(dest!=0x08 && dest!=0x04) return 1;
    if((clk!=0) && (clk!=1) && (clk!=2)) return 1;
/*    sound_conf.sound_source = source;*/
/*    sound_conf.sound_dest = dest;*/
    
    sound_conf.sound_clk = clk;
    if(prescale!=0) 
    {
    	sound_conf.sound_prescale = prescale;
    	
    }
    else 
    {	sound_conf.sound_prescale = sound_conf.soundcmd_prescale;
   	sound_conf.sound_clk = 0; 
    }
    if(sound_conf.sound_prescale < 1) sound_conf.sound_prescale = 1;
/*    sound_conf.sound_protocol = protocol;*/
    return 0;
}

void amiga_buffptr(LONG ptr)
{
   struct SndBufPtr {
        LONG play;
        LONG record;
    ...
    sbp->play = sound_conf.current_sound_addr+4L;
}

/* mode : bits 0 : DMA playback 1: loop playback */
LONG amiga_buffoper(WORD mode)
{   
  /*  if(mode>3) return 1; */ /* play only */
    
    if(mode==-1) return sound_conf.sound_state;
    else 
    {
    	if (!(mode&1)) /* stop sound */
    	{
    	    if(mfp_emu.tacr &0x08) jdisint(13);
    	    else jdisint(15);
    	    sound_conf.sound_state = 0;
#ifdef WITH_END_BUFFER
    	    if(sound_conf.oneshot==0) amiga_setbuffer(0, ((ULONG)endbuffer+7L)&0xFFFFFFF8L, (((ULONG)endbuffer+7L)&0xFFFFFFF8L)+8L);
    	    else
#endif
    	    {
    	      INTENA = CLRBITS | AUD0; /* stop interrupt */
    	      DMACON = CLRBITS | AUD0EN; /*0x0001 Arret canal 0 */
    	    }
   ...
   /* 	if(!(sound_conf.sound_state&1))*/
    	{   ULONG atari_rate, ref_freq=0L, calc_AUDPER; 
    		if(sound_conf.sound_startaddr==0L) return 1;  
    		switch (sound_conf.sound_clk)
    		{
    			case 0: ref_freq = 98340L;
    			break;
    			case 2: ref_freq = 125000L;
   	 		break;
   	 		case 1:
   	 			if(sound_conf.gpio_data&1) ref_freq = 48000L*2L;
   	 			else ref_freq = 44100L*2L;
   	 		break;
   	 		default:
   	 		break;
   	 	}
   	 	if(ref_freq==0) return 1;
   	 	atari_rate = ref_freq / (sound_conf.sound_prescale+1); /* sample rate request Hz */ 
   	 	calc_AUDPER = (3528000L/atari_rate);
   	 	if(calc_AUDPER<63) calc_AUDPER=63; /* mini possible */
   	 	INTENA = CLRBITS | AUD0; /* stop interrupt */
   	        DMACON = CLRBITS | AUD0EN;  /*0x0001 Arret canal 0 */


	        AUD0PERSAGA = (UWORD)calc_AUDPER; /* 63 :56 khz, 80 : 44,1 khz */
	        AUD0VOLSAGA = sound_conf.sound_vol; /* volume son */
	        ...
	        {
	            sound_conf.oneshot=2;
	        }
	     /*   sound_conf.oneshot=0;*/
	        switch (sound_conf.sound_mode)
	        {
	    	    case 0:
	    	        AUD0CTRLSAGA=0x1|sound_conf.oneshot; /* 8 bit stereo doesn't exist do as 16 bit mono*/
	    	    break;
	    	    case 1: /* 16 bit stereo */
	    	        AUD0CTRLSAGA=0x5|sound_conf.oneshot;
	    	    break;
	    	    case 2: /* 8 bit mono */
	    	        AUD0CTRLSAGA=sound_conf.oneshot;
	    	    break;
	    	    case 3: /* 16 bit mono */
	    	    	AUD0CTRLSAGA=0x1|sound_conf.oneshot;
	    	    break;
	    	    default:
	    	    	return 1;
	    	    break;
	        ...
	        sound_conf.current_sound_addr = sound_conf.last_sound_addr;
	        sound_conf.sound_state = mode&3;
	        if(sound_conf.oneshot==0) sound_conf.start_song=1;
#if NO_AMIGA_VBI
	        INTENA = CLRBITS | AUD0; /* start interrupt sound chanel 0 always running for emulation*/
        	DMACON = CLRBITS | AUD0EN; /* 0x8001 canal 0*/
#else
            INTENA = SETBITS | AUD0; /* start interrupt sound chanel 0 always running for emulation*/
            DMACON = SETBITS | AUD0EN; /* 0x8001 canal 0*/
#endif
	}
...
LONG amiga_sndstatus(WORD mode)
{
  switch (mode)
  {
  	case 0: return 0;
  	...
  	case 1: 
  	  ...
  	  return 0; /* reset */
  	break;
  	case 2: return 3; /* 8 & 16 bit supported only */
  	break;
  	case 3: return 2; /* attenuation DAC only */
  	break;
  	case 4: return 0; /* record source */
  	break;
  	case 5: return 0; /* no full duplex */
  	break;
  	case 8: return 1; /* 8 bit signed format */
  	break;
  	case 9: return 5; /* 16 bit bigendian signed */
  	break;
  	case 10: return ENODEV; /* 24 bit not supported */
  	break;
  	case 11: return ENODEV; /* 32 bit not supported */
  	break;
  	default:
  	break;
  }
...
LONG amiga_soundcmd(WORD mode, WORD data) /* mode : 0 Set attenuation of the left output channel in 1.5 dB increments 1: right */ 
{   UWORD  table_db[16]={128,119,111,102,94,85,77,68,60,51,43,34,26,17,9,0} /* table pour SAGA*/, val;
    /*table_db[16]={64,54,46,38,32,26,21,18,16,14,12,10,34,6,4,2} table pour Amiga standard*/
  LONG rep=0;
  switch (mode)
  {
    case 0:
    	...
    	{
    	  data&=0x00f0;
    	  sound_conf.sound_vol_atari=(sound_conf.sound_vol_atari&0xf0ff) | (data<<4);
    	  data>>=4;
	  val = table_db[data];
          
          sound_conf.sound_vol&=0x00FF;
          sound_conf.sound_vol|=(val<<8);
          
          AUD0VOLSAGA = sound_conf.sound_vol;
        }
        return (sound_conf.sound_vol_atari >> 4) & 0x00f0;
    break;
    case 1:
    	if(data!=-1)
    	{
    	  data&=0x00f0;
    	  sound_conf.sound_vol_atari=(sound_conf.sound_vol_atari&0xff0f) | data;
    	  data>>=4;
	  val = table_db[data];

          sound_conf.sound_vol&=0xFF00;
          sound_conf.sound_vol|=val;
          
          AUD0VOLSAGA = sound_conf.sound_vol;
        }
        return sound_conf.sound_vol_atari & 0x00f0;
    break;
    case 6:
	if(data >= 0 && data <= 3)
	{	
		switch (data)
		{
			case 0: sound_conf.soundcmd_prescale = 14; /* 6 khz */
			break;
			case 1: sound_conf.soundcmd_prescale = 7; /* 12 khz */
			break;
			case 2: sound_conf.soundcmd_prescale = 3; /* 25khz */
			break;
			case 3: sound_conf.soundcmd_prescale = 1; /* 50Khz */
			break;
		} 
		rep = sound_conf.soundcmd_prescale;
	}
   break;
   case 7: /* frequency */
   	if (data==-1) return 0;
   	{ long ref_freq;
   		switch (sound_conf.sound_clk)
    		{
    			case 0: ref_freq = 98340;
    			break;
    			case 2: ref_freq = 125000;
   	 		break;
   	 		default:
   	 			ref_freq = 0;
   	 		break;
   	 	}
   	 	if(ref_freq==0) return 0;
   	 	sound_conf.sound_rate = (unsigned short)data ;
   	 	if(sound_conf.sound_rate) sound_conf.sound_prescale = (ref_freq / ((long)sound_conf.sound_rate ))-1;
   	 	rep = ref_freq / (sound_conf.sound_prescale+1); /* sample rate request Hz */ 
   	}
   break;
   default : break;
   }
...
static void amiga_ikbd_simulate_writeb(UBYTE b)
{
    ...
    *p++ = b;
    ...
    {
        case 0x07: /* SET MOUSE BUTTON ACTION */
            if((buffer[1] & 0x7) == 0x04)
            {
                mouse_report_kd_left  = mouse_report_0x74;
                mouse_report_kd_right = mouse_report_0x75;
                mouse_report_ku_left  = mouse_report_0xf4;
                mouse_report_ku_right = mouse_report_0xf5;
            }
            else
            {
                ...
                mouse_report_ku_right = mouse_report_none;

                if(buffer[1] & 0x01)
                {
                    mouse_report_kd_left  = mouse_report_absolute;
                    mouse_report_kd_right = mouse_report_absolute;
                }

                if(buffer[1] & 0x02)
                {
                    mouse_report_ku_left  = mouse_report_absolute;
                    mouse_report_ku_right = mouse_report_absolute;
                }
            }
        break;

        case 0x09: /* SET ABSOLUTE MOUSE POSITIONING */
            // ideally we should disable interrupts here
            mousexmax = (buffer[1] << 8) | buffer[2];
            mouseymax = (buffer[3] << 8) | buffer[4];
            absbuttons = 0;
        ...
        case 0x0e: /* LOAD MOUSE POSITION */
            // ideally we should disable interrupts here
            mousex = (buffer[2] << 8) | buffer[3];
            mousey = (buffer[4] << 8) | buffer[5];
            // ... and reenable them here
        break;
    ...
    if(mouse_events_disabled || port0_joystick_mode)
        mouse_report_mv = mouse_report_none;
    else
    ...
    if(joysticks_events_disabled)
    {
        ...
        joy1_report = joy1_report_off; 
    }
    else
    {
        joy0_report = port0_joystick_mode ? joy0_report_event : joy0_report_off;
...
const UBYTE scancode_atari_from_amiga[128] =
{
    ...
    0x47, 0x52/* end pc key map on insert*/, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static UBYTE oldWheel;
static UBYTE oldMouseX;
static UBYTE oldMouseY;
static BOOL oldButton1;
static BOOL oldButton2;
static BOOL oldButton3;
static BOOL oldButton4;
static BOOL oldButton5;
static BOOL oldMouseSet;

void amiga_mouse_poll(void)
{
    UWORD data,wdata, ciaapra, potgor;
    UBYTE mouseX, mouseY, wheel;
    ...
    button2 = !(ciaapra & 0x80) || !(potgor & 0x0400);
    button3 = !(potgor & 0x0100);
    button4 = !(wdata & 0x100);
    button5 = !(wdata & 0x200);
    ...
    if(oldMouseSet)
    {
        ...
        if(button3 != oldButton3)
            call_kbdvec(0x37 | (button3 ? 0x00 : 0x80));

        if(button4 != oldButton4)
            call_kbdvec(0x5e | (button4 ? 0x00 : 0x80));

        if(button5 != oldButton5)
            call_kbdvec(0x5f | (button5 ? 0x00 : 0x80));
    }
...
#if CONF_WITH_APOLLO_68080
#define AMIGA_CLOCK_DS3231 3
#endif

static UBYTE amiga_i2c_buf[19];
...
#if CONF_WITH_APOLLO_68080
    /* Probe for I2C RTC by reading seconds */
    amiga_i2c_buf[0] = 0x00;
    if( (amiga_sendI2C(0xd0, 1, amiga_i2c_buf) == 0) &&
        (amiga_recvI2C(0xd0, 1, amiga_i2c_buf) == 0))
    {
        amiga_clock_type = AMIGA_CLOCK_DS3231;
    }
    else
#endif
...
void amiga_setdt(ULONG dt)
{
    UWORD date = HIWORD(dt);
    UWORD year  = date >> 9;
    UWORD month = (date >> 5) & 0x0f;
    UWORD day   = date & 0x1f;

    UWORD time = LOWORD(dt);
    UWORD hours   = time >> 11;
    UWORD minutes = (time >> 5) & 0x3f;
    UWORD seconds = (time << 1) & 0x3f;

#if CONF_WITH_APOLLO_68080
    if (amiga_clock_type == AMIGA_CLOCK_DS3231)
    {
        amiga_i2c_buf[0] = 0x00;

        amiga_i2c_buf[7] = int2bcd(year);
        amiga_i2c_buf[6] = int2bcd(month);
        amiga_i2c_buf[5] = int2bcd(day);
        amiga_i2c_buf[4] = 1; /* dow - not used */
        
        amiga_i2c_buf[3] = int2bcd(hours);
        amiga_i2c_buf[2] = int2bcd(minutes);
        amiga_i2c_buf[1] = int2bcd(seconds);

        (void)amiga_sendI2C(0xd0, 7 + 1, amiga_i2c_buf);
    }
#endif

    KDEBUG(("amiga_setdt(0x%lx)\n", dt));
}
...
ULONG amiga_getdt(void)
{
    ...
    if (amiga_clock_type == AMIGA_CLOCK_DS3231)
    {
        ...
        if(amiga_recvI2C(0xd0,7, amiga_i2c_buf) != 0)
            return DEFAULT_DATETIME;

        seconds = bcd2int(amiga_i2c_buf[0]);
        minutes = bcd2int(amiga_i2c_buf[1]);
        hours   = bcd2int(amiga_i2c_buf[2] & 0x3f); /* mask off AM/PM bit */
        days    = bcd2int(amiga_i2c_buf[4]);
        months  = bcd2int(amiga_i2c_buf[5] & 0x7f); /* mask off century bit */
        years   = bcd2int(amiga_i2c_buf[6]);
    }
    else
#endif
    {
        UWORD offset  = (amiga_clock_type == AMIGA_CLOCK_RF5C01A) ? 1 : 0;
        seconds = read_clock_bcd(0);
        minutes = read_clock_bcd(2);
        hours   = read_clock_bcd(4);
        days    = read_clock_bcd(offset + 6);
        months  = read_clock_bcd(offset + 8);
        years   = read_clock_bcd(offset + 10);
    ...
    KDEBUG(("amiga_getdt() = 0x%lx\n", MAKE_ULONG(date, time)));
 ...
 * https://github.com/tonioni/WinUAE/blob/master/autoconf.cpp */
#if 0  /* xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx a remettre enlev‚ pour GCC 3*/
...
#if 0 /*xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx a remettre enlever pour GCC 3*/
...
void amiga_isr_tbe(void);
void amiga_isr_rbf(void);
void amiga_rs232_init(void)
{
#if CONF_SERIAL_IKBD
    VEC_MFP_SBE = amiga_isr_tbe;
    VEC_MFP_RBF = amiga_isr_rbf;
...
void amiga_rs232_tbe_interrupt(void)
{
}
...
void amiga_sound_vector(WORD numIT, LONG vector)
{/*	static short it15prio=0;
	if(numIT==15) 
	{
		sound_int_call = vector;
		it15prio=1;
	}
	if((numIT==13) && (it15prio==0)) 
	{
		sound_int_call = vector;
	}*/
	sound_int_call = vector;
...
void amiga_setup_timer(WORD timer)
{
    static const ULONG mfp_div[8] = { 0, 4, 10, 16, 50, 64, 100, 200, };
    UBYTE ctrl;
    ULONG data;
    ...
    switch(timer)
    {
        case 0: ctrl = mfp_emu.tacr; CIAACRB &= ~0x5f; data = mfp_emu.tadr; break; /* MFP Timer A = CIAA, Timer B */ 
        case 1: ctrl = mfp_emu.tbcr; CIABCRA &= ~0x3f; data = mfp_emu.tbdr; break; /* MFP Timer B = CIAB, Timer A */
        case 2: ctrl = (mfp_emu.tcdcr >> 4) & 0x07; /* MFP Timer B = CIAB, Timer B */
                CIABCRB &= ~0x5f; data = mfp_emu.tcdr;
                break;

        case 3: /* MFP Timer D = not mapped */
        default:
            return;
    }
    ...
    if(ctrl & 0x08)
    {
        /* Timer configured for event mode; we don't support this yet as this likely require a different mechanism */
        return;
    }
    
    data = (10 * data * mfp_div[ctrl & 0x07]) / 35; /* Rough MFP * prescaler (div) vs. CIA freq * prescaler (10) */
    ...
    switch(timer)
    {
        case 0: CIAATBLO = LOBYTE(data); CIAATBHI = HIBYTE(data); CIAACRB |= 0x11; break; /* MFP Timer A = CIAA Timer B */
        case 1: CIABTALO = LOBYTE(data); CIABTAHI = HIBYTE(data); CIABCRA |= 0x11; break; /* MFP Timer B = CIAB Timer A */
        case 2: CIABTBLO = LOBYTE(data); CIABTBHI = HIBYTE(data); CIABCRB |= 0x11; break; /* MFP Timer C = CIAB Timer B */
        case 3: break; /* MFP Timer D = not mapped */
        default:
            break;
    }
}

void amiga_jenabint(WORD num)
{
    if((num==13) && (mfp_emu.tacr & 0x08))
    {
        /* Timer configured for event mode; we don't support this yet as this likely require a different mechanism */
        if(sound_int_call) play_int_ch0=1; /* start interrupt sound chanel 0 */
        return;
    }
   
    switch(num)
    {
        case 13: CIAAICR = 0x82; INTENA = SETBITS | INTEN; break; /* MFP Timer A = CIAA Timer B */
        case 8:  CIABICR = 0x81; INTENA = SETBITS | EXTER; break; /* MFP Timer B = CIAB Timer A */
        case 5:  CIABICR = 0x82; INTENA = SETBITS | EXTER; break; /* MFP Timer C = CIAB Timer B */
        case 15: if(sound_int_call) play_int_ch0=1;   break; /* start interrupt sound chanel 0 */
        default:
            break;
    }
}

void amiga_jdisint(WORD num)
{ 
    switch(num)
    {
        case 13: CIAAICR = 0x02; if(mfp_emu.tacr & 0x08) play_int_ch0=0; break;  /* MFP Timer A = CIAA Timer B */
        case 8:  CIABICR = 0x01; break; /* MFP Timer B = CIAB Timer A */
        case 5:  CIABICR = 0x02; break; /* MFP Timer C = CIAB Timer B */
        case 15: play_int_ch0=0; break;
        default:
            break;
    }
}

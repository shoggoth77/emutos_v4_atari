...
struct dmasound
{
    ...
    UBYTE frame_start_high32; /* Frame start address (high byte) */
#else
    UBYTE filler02;
#endif
...
#if defined(MACHINE_AMIGA)
struct dmasound amiga_dma_sound;
struct dmasound *DMASOUND=&amiga_dma_sound;
#else
...
#if defined(MACHINE_AMIGA)
    has_dmasound=1;
    has_falcon_dmasound = 1;
    has_microwire = 0;
  /*  DMASOUND =  &amiga_dma_sound;*/
...
static LONG sndcmd_falcon(WORD mode, WORD data)
{
    UWORD val;
#if 0
#if defined(MACHINE_AMIGA) /* !!!!!!!!!!! à éliminer lorsque la partie amiga singera les registres du falcon */
  amiga_soundcmd(mode, data);
#endif
#endif
    switch (mode)
    {
...
#if defined(MACHINE_AMIGA)
    DMASOUND->frame_start_high32 = (UBYTE)(startaddr >> 24);
#endif
...
#if defined(MACHINE_AMIGA)
    return amiga_setbuffer(mode, startaddr, endaddr);
#else
    return 0;
#endif
 ...
 */
#if defined(MACHINE_AMIGA)
LONG setinterrupt(UWORD mode, WORD cause, void *intr_addr)
{
    ...
    return amiga_setinterrupt(mode, cause, intr_addr);
...
LONG buffoper(WORD mode)
{
    ...
    else DMASOUND->control = ((mode & 0xc) << 2) | (mode & 0x3);
#if defined(MACHINE_AMIGA)
    return amiga_buffoper(mode);
#else
    return 0;
#endif
}
...
#if defined(MACHINE_AMIGA)
    (void)devconnect_falcon(source, dest, clk, prescale, protocol);
    return amiga_devconnect(source, dest, clk, prescale, protocol);
#else
    if (has_falcon_dmasound)
